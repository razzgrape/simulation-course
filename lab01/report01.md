...
Данное приложение моделирует траекторию движения тела, запущенного под углом к горизонту в земной атмосфере. Расчет производится с учетом силы тяжести и квадратичного сопротивления воздуха.
...
## 3. Численное решение (Метод Эйлера)

Так как ускорение зависит от скорости, задача решается численным методом интегрирования с шагом по времени $\Delta t$. В программе реализован модифицированный метод Эйлера (Эйлер-Кромер).
...
## 4. Коррекция точки приземления

Для исключения ошибки «пролета сквозь землю» при больших шагах $\Delta t$ используется **линейная интерполяция**. 

Если на текущем шаге координата $y_{i+1}$ стала отрицательной, вычисляется доля шага `fraction` до момента касания земли ($y = 0$):
$$fraction = \frac{y_i}{y_i - y_{i+1}}$$

Это позволяет скорректировать финальную дальность $x$ и время полета, обеспечивая высокую точность данных даже при грубых настройках симуляции.
...
Использование крупного шага (1.0 с) ведет к занижению дальности полета более чем на 25% из-за редкого пересчета ускорения. 
Внедренная линейная интерполяция точки приземления позволяет корректно завершать траекторию ровно на высоте 0 м, устраняя физически неверный "провал" снаряда под землю